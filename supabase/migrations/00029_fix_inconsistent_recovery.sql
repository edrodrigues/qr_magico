-- Caso D: só falhar estado inconsistente após grace period (evita falso positivo em retry).
...
BEGIN
  ...
  -- D: estado inconsistente — só após grace period (não durante geração ativa)
  RETURN QUERY
  UPDATE public.presentes p
  SET
    status = 'failed',
  ...
  WHERE m.presente_id = p.id
    ...
    AND COALESCE(p.generation_started_at, p.created_at) < now() - (max_age_minutes || ' minutes')::interval
